...
IS_SHADOW = os.getenv("IS_SHADOW", "false").lower() in ("true", "1", "yes")
...
letter_offset = year_offset + 7 if IS_SHADOW else year_offset
...
# Cleanup commit_log file (clear its contents)
with open(COMMIT_FILE, "w") as f:
    pass
run_cmd(f'git add "{COMMIT_FILE}"')
run_cmd('git commit -m "Cleanup commit log"')
